...
Step "version sync" {
    ...
    if ($init -notmatch '1\.0\.30') { throw "__init__.py not 1.0.30" }
    if ($pyproj -notmatch '1\.0\.30') { throw "pyproject.toml not 1.0.30" }
    $iss = Get-Content (Join-Path $Root "packaging\HonestSpend.iss") -Raw
    if ($iss -notmatch '1\.0\.30') { throw "HonestSpend.iss not 1.0.30" }
    Write-Host "version 1.0.30 consistent"
}
