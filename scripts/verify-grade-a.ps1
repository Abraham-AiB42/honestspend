...
Step "version sync" {
    ...
    if ($init -notmatch '1\.0\.4') { throw "__init__.py not 1.0.4" }
    if ($pyproj -notmatch '1\.0\.4') { throw "pyproject.toml not 1.0.4" }
    $iss = Get-Content (Join-Path $Root "packaging\LedgerRing.iss") -Raw
    if ($iss -notmatch '1\.0\.4') { throw "LedgerRing.iss not 1.0.4" }
    Write-Host "version 1.0.4 consistent"
}
