# HonestSpend — scheduled auto-backup (run by Task Scheduler)
...
$args = @("-m", "honestspend.cli", "backup", "--auto")
if ($env:FOS_BACKUP_FORCE -eq "1") {
  $args = @("-m", "honestspend.cli", "backup", "--force", "--note", "task-scheduler", "--keep", "14")
}
