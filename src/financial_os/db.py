...
class AppSettings(Base):
    ...
    product_name: Mapped[str] = mapped_column(String(64), default="LedgerRing")
...
def init_db(engine=None) -> None:
    ...
    from financial_os.migrations import run_migrations

    run_migrations(eng)
    # Apply pending staged restore (safe backup restore from prior process)
    try:
        from financial_os.services.backup import apply_pending_restore
