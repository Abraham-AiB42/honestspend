...
def _next_db_path() -> Path:
    return settings.data_dir / "financial_os.db.next"
...
def _extract_backup_to(path: Path, dest_db: Path) -> None:
    if path.suffix == ".zip":
        with zipfile.ZipFile(path, "r") as zf:
            names = zf.namelist()
            db_member = next(
                (n for n in names if n.endswith("financial_os.db") or n.endswith(".db")),
                None,
            )
            if not db_member:
...
def restore_from_backup(name: str) -> dict[str, Any]:
    """
    Stage restore: write financial_os.db.next + .restore_pending.
