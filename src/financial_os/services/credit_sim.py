...
Uses only data LedgerRing knows (accounts, balances, user-supplied history inputs).
...
from financial_os.services.debt_strategy import DebtView, utilization
...
def simulate_credit(
    ...
    *,
    ...
    return CreditSimResult(
        model="LedgerRing educational model (VantageScore 3.0-style weights)",
        disclaimer=(
            ...
            "This simulation uses only debts and history you track in LedgerRing for planning."
        ),
        score=score,
        band=_band(score),
        factors=factors,
