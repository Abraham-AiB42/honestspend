...
from financial_os.services.backup import create_backup, backups_dir
...
def get_remote_config() -> dict[str, Any]:
    ...
    if not p.is_file():
        return {
            ...
            "hint": "Set a folder (e.g. OneDrive\\LedgerRing\\backups) for encrypted snapshot copies.",
        }
    try:
...
def decrypt_bytes(blob: bytes, password: str) -> bytes:
    ...
    if not blob.startswith(MAGIC):
        raise ValueError("Not a LedgerRing encrypted backup (bad magic)")
    salt = blob[4:20]
    nonce = blob[20:32]
    ct = blob[32:]
    key = _derive_key(password, salt, PBKDF2HMAC, hashes)
