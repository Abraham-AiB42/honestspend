...
When app lock is enabled, books are sealed to ``honestspend.db.sealed`` and the
plaintext ``honestspend.db`` is removed. Unlock derives/unwraps a DEK and
...
def refuse_offline_open_if_encrypted() -> None:
    ...
    Prevents creating an empty honestspend.db next to a real .sealed vault.
    """
    if not encryption_enabled():
        return
