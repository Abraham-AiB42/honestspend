...
from honestspend import __version__
from honestspend.config import settings
from honestspend.config import settings as app_settings
...
from honestspend.services.demo_seed import seed_demo_if_empty
...
from honestspend.services.payoff import plan_card_payoff, plan_to_dict
...
async def lifespan(_app: FastAPI):
    ...
    try:
        from honestspend.services.secrets_store import apply_secrets_to_settings
    ...
    if boot.get("unlocked"):
        _seed_if_open()
        try:
            from honestspend.services.auto_backup import start_auto_backup_loop
    ...
    try:
        from honestspend.services.auto_backup import stop_auto_backup_loop
...
async def permission_middleware(request: Request, call_next):
    ...
    session = factory()
    try:
        from honestspend.services.permissions import (
...
@app.get("/api/license")
def license_status():
    from honestspend.services.license_service import get_status
...
def license_activate(body: LicenseActivateIn):
    from honestspend.services.license_service import activate_key
...
def license_store(body: LicenseStoreIn):
    ...
    from honestspend.services.license_service import activate_store
...
def budgets_list(profile_id: int | None = None, db: Session = Depends(get_db)):
    from honestspend.services.budget_service import list_rules
...
def budgets_suggestions_api(
    ...
    from honestspend.services.budget_service import suggestions
...
def budgets_accept_suggestion(body: BudgetAcceptSuggestionIn, db: Session = Depends(get_db)):
    from honestspend.services.budget_service import create_rule, suggest_for_category
...
def budgets_seed_from_history(body: BudgetSeedIn | None = None, db: Session = Depends(get_db)):
    ...
    from honestspend.services.budget_service import seed_from_history
...
def budgets_cuts_preview(
    ...
    from honestspend.services.budget_service import preview_cuts
...
def budgets_cuts_apply(body: BudgetCutIn, db: Session = Depends(get_db)):
    from honestspend.services.budget_service import apply_cut
...
def license_clear():
    from honestspend.services.license_service import clear_license
...
def license_refresh():
    from honestspend.services.license_service import refresh_license
...
def onboarding_status(db: Session = Depends(get_db)):
    from honestspend.services.onboarding import get_onboarding_status
    from honestspend.services.setup_wizard import get_setup_state
...
def setup_advance(body: SetupAdvanceIn, db: Session = Depends(get_db)):
    from honestspend.services.setup_wizard import advance_setup
...
def setup_storage_post(body: SetupStorageIn, db: Session = Depends(get_db)):
    ...
    from honestspend.services.paths import apply_storage_choice
    from honestspend.services.setup_wizard import set_phase
...
def setup_security_get(platform: str = "win", db: Session = Depends(get_db)):
    ...
    from honestspend.services.app_security import security_catalog
    from honestspend.services.setup_wizard import get_setup_state
...
def setup_security_post(body: SetupSecurityIn, db: Session = Depends(get_db)):
    ...
    from honestspend.services.app_security import validate_security_payload
    from honestspend.services.setup_wizard import set_phase
...
def setup_complete(body: SetupSkipIn | None = None, db: Session = Depends(get_db)):
    ...
    from honestspend.services.setup_wizard import mark_setup_done
...
def setup_cash_status(profile_id: int | None = None, db: Session = Depends(get_db)):
    ...
    from honestspend.services.setup_cash import cash_accounts_status
...
def setup_cash_account_create(body: SetupCashAccountIn, db: Session = Depends(get_db)):
    ...
    from honestspend.services.setup_cash import create_cash_account
...
def setup_discover(
    ...
    from honestspend.services.setup_discover import discover_liabilities
...
def account_payment_option(
    ...
    from honestspend.services.setup_discover import set_account_payment_option
...
def setup_recurring_remaining(
    ...
    from honestspend.services.setup_recurring_finalize import remaining_recurring
...
def setup_categorize_status(
    ...
    from honestspend.services.setup_categorize import categorize_status
...
def setup_categorize_auto(body: SetupCategorizeAutoIn | None = None, db: Session = Depends(get_db)):
    from honestspend.services.setup_categorize import auto_apply_high_confidence, categorize_status
...
def setup_categorize_confirm(body: SetupCategorizeConfirmIn, db: Session = Depends(get_db)):
    from honestspend.services.setup_categorize import confirm_payee_category
...
def setup_budgets_review(
    ...
    from honestspend.services.setup_budgets import budgets_review
...
def setup_budgets_apply(body: SetupBudgetEditsIn, db: Session = Depends(get_db)):
    from honestspend.services.setup_budgets import apply_budget_edits, budgets_review
...
def setup_buffers_get(profile_id: int | None = None, db: Session = Depends(get_db)):
    from honestspend.services.setup_budgets import buffers_status
...
def setup_buffers_save(body: SetupBuffersIn, db: Session = Depends(get_db)):
    from honestspend.services.setup_budgets import save_buffers
...
def onboarding_complete(db: Session = Depends(get_db)):
    from honestspend.services.onboarding import complete_onboarding
...
def onboarding_quick_setup(body: QuickSetupIn, db: Session = Depends(get_db)):
    from honestspend.services.onboarding import apply_quick_setup
...
def archive_profile_api(profile_id: int, db: Session = Depends(get_db)):
    from honestspend.services.profiles import archive_profile
...
def _apply_payment_option_alias(row: Account) -> None:
    ...
    """
    from honestspend.services.autopay import PAYMENT_OPTION_TO_POLICY
...
def archive_account(account_id: int, db: Session = Depends(get_db)):
    ...
    if (row.kind or "") == "credit":
        from honestspend.services.autopay import _end_card_payment_schedules
...
def _cycle_api_dict(proj: dict, account: Account, db: Session | None = None) -> dict:
    ...
    from honestspend.services.card_honesty import honesty_payload
...
def get_account_peak(
    ...
    from honestspend.services.account_peak import peak_balance
...
def put_account_cycle_config(
    ...
    if "payment_timing" in data and data["payment_timing"] is not None:
        from honestspend.services.statement_cycle import TIMINGS
...
def list_account_promo_lines(account_id: int, db: Session = Depends(get_db)):
    ...
    from honestspend.services.promo_installments import line_to_dict, list_promo_lines
...
def patch_account_promo_line(
    ...
    from honestspend.services.autopay import recompute_card_payment_schedule
    from honestspend.services.promo_installments import line_to_dict, update_promo_line
...
def roll_account_promo_line(
    ...
    from honestspend.services.autopay import recompute_card_payment_schedule
    from honestspend.services.promo_installments import line_to_dict, roll_line
...
def list_scheduled_series(
    ...
    from honestspend.services.schedule_series import list_series
...
def step_scheduled_series(body: ScheduleSeriesStepIn, db: Session = Depends(get_db)):
    ...
    from honestspend.services.schedule_series import add_series_step
...
def get_statement_cycles_history(account_id: int, limit: int = 24, db: Session = Depends(get_db)):
    from honestspend.services.statement_freeze import list_statement_cycles
...
def rewards_pick_card(
    ...
    from honestspend.services.rewards_pick import pick_card_for_category
...
def put_account_rewards_rates(
    ...
    from honestspend.services.rewards_pick import parse_rewards_rates, set_rewards_rates
...
def get_account_rewards_rates(account_id: int, db: Session = Depends(get_db)):
    from honestspend.services.rewards_pick import parse_rewards_rates, rates_for_account
...
def transfer_candidates(
    ...
    from honestspend.services.transfer_match import find_transfer_candidates
...
def import_presets_list(db: Session = Depends(get_db)):
    from honestspend.services.import_presets import list_presets
...
def import_presets_put(body: ImportPresetIn, db: Session = Depends(get_db)):
    from honestspend.services.import_presets import upsert_preset
...
def tax_vault_put(body: TaxVaultIn, db: Session = Depends(get_db)):
    from honestspend.services.tax_vault import set_tax_vault
...
def tax_vault_adjust(body: TaxVaultAdjustIn, db: Session = Depends(get_db)):
    from honestspend.services.tax_vault import adjust_tax_vault
...
def fees_confirm(body: FeeConfirmIn, db: Session = Depends(get_db)):
    from honestspend.services.fee_scan import confirm_fee
...
def fees_summary(
    ...
    from honestspend.services.fee_scan import fee_summary
...
def autopay_list(profile_id: Optional[int] = None, db: Session = Depends(get_db)):
    from honestspend.services.autopay import list_autopay
...
def autopay_set(account_id: int, body: AutopayIn, db: Session = Depends(get_db)):
    from honestspend.services.autopay import set_autopay
...
def digest_brief(
    ...
    from honestspend.services.digest_brief import build_fiscal_brief
...
def glance(
    ...
    from honestspend.services.glance import build_glance
...
def reports_cashflow(
    ...
    from honestspend.services.reports import cashflow_by_entity
...
def reports_debt(
    ...
    from honestspend.services.reports import debt_snapshot
...
def tax_year_checklist(
    ...
    from honestspend.services.tax_year import build_tax_year_checklist
...
def scenarios_list(
    ...
    from honestspend.services.scenarios import list_scenarios
...
def scenarios_create(body: ScenarioIn, db: Session = Depends(get_db)):
    from honestspend.services.scenarios import create_scenario
...
def scenarios_run(scenario_id: int, db: Session = Depends(get_db)):
    from honestspend.services.scenarios import run_scenario
...
def scenarios_delete(scenario_id: int, db: Session = Depends(get_db)):
    from honestspend.services.scenarios import delete_scenario
...
def scenarios_quick(body: ScenarioQuickIn, db: Session = Depends(get_db)):
    ...
    from honestspend.services.scenarios import quick_scenario_from_amount
...
def home_month_close_complete(
    ...
    from honestspend.services.month_close import mark_month_closed
...
def payment_confirm(body: PaymentConfirmIn, db: Session = Depends(get_db)):
    from honestspend.services.payment_match import apply_payment_as_transfer
...
def permission_roles():
    from honestspend.services.permissions import list_roles
...
def permission_me(request: Request, db: Session = Depends(get_db)):
    from honestspend.services.permissions import CAPS, auth_status

    ctx = getattr(request.state, "access", None)
    if ctx is None:
        from honestspend.services.permissions import default_context
...
def permission_auth_status(db: Session = Depends(get_db)):
    from honestspend.services.permissions import auth_status
...
def permission_audit(limit: int = Query(50, ge=1, le=200), db: Session = Depends(get_db)):
    from honestspend.services.audit import list_events
...
def permission_users(db: Session = Depends(get_db)):
    from honestspend.db import AppUser
    from honestspend.services.permissions import auth_status
...
def create_user(body: UserIn, db: Session = Depends(get_db)):
    from honestspend.db import AppUser
    from honestspend.services.permissions import Role, generate_api_token
    ...
    db.add(row)
    db.flush()
    from honestspend.services.permissions import auth_status, generate_api_token as _gen
...
def rotate_token(user_id: int, db: Session = Depends(get_db)):
    from honestspend.db import AppUser
    from honestspend.services.permissions import generate_api_token
...
def credit_health(db: Session = Depends(get_db)):
    from honestspend.services.debt_service import run_credit_health
...
def credit_profile_get(db: Session = Depends(get_db)):
    from honestspend.services.debt_service import get_credit_profile
...
def debt_plan(body: DebtPlanRequest, db: Session = Depends(get_db)):
    from honestspend.services.debt_service import run_debt_plan
...
def debt_compare(
    ...
    from honestspend.services.debt_service import run_strategy_compare
...
def test_rule_pattern(body: RuleTestIn, db: Session = Depends(get_db)):
    ...
    from honestspend.services.categorizer import match_rule
...
def tax_readiness(
    ...
    from honestspend.services.tax_packet import tax_packet_readiness
...
def import_xlsx_path(body: ImportPathIn, db: Session = Depends(get_db)):
    from honestspend.services.paths_safe import resolve_under_data_dir
...
def import_bank_guides():
    ...
    from honestspend.services.bank_guides import list_bank_guides
...
def import_bank_guide_one(guide_id: str):
    from honestspend.services.bank_guides import get_bank_guide
...
def import_inbox_status():
    from honestspend.services.import_inbox import ensure_inbox_layout, list_inbox_files
...
def import_inbox_process(body: InboxProcessIn, db: Session = Depends(get_db)):
    from honestspend.services.import_inbox import process_inbox
...
async def import_ofx_preview(file: UploadFile = File(...)):
    ...
    content = await file.read()
    from honestspend.services.bank_ofx import preview_ofx
    from honestspend.services.paths_safe import enforce_upload_size
...
def reconcile_get(
    ...
    from honestspend.services.reconcile import reconcile_report
...
def plaid_status(db: Session = Depends(get_db)):
    from honestspend.services.secrets_store import (
...
def plaid_credentials_get():
    from honestspend.services.secrets_store import apply_secrets_to_settings, plaid_credentials_status
...
def plaid_credentials_save(body: PlaidCredentialsIn):
    ...
    from honestspend.services.secrets_store import save_plaid_credentials
...
def plaid_credentials_delete():
    from honestspend.services.secrets_store import clear_plaid_credentials
...
def ai_credentials_list():
    from honestspend.services.secrets_store import apply_secrets_to_settings, get_ai_providers
...
def ai_credentials_save(body: AiCredentialsIn):
    from honestspend.services.secrets_store import save_ai_credentials
...
def ai_credentials_delete(provider: str):
    from honestspend.services.secrets_store import clear_ai_credentials
...
def import_reminder_snooze(body: ImportSnoozeIn, db: Session = Depends(get_db)):
    from honestspend.services.import_reminders import snooze_import_reminder
...
def system_info():
    from honestspend.services.backup import db_status
    from honestspend.services.paths import data_path_info
...
def backup_status(db: Session = Depends(get_db)):
    from honestspend.services.backup import db_status, list_backups, schedule_status
...
def backup_schedule_get(db: Session = Depends(get_db)):
    from honestspend.services.backup import schedule_status
...
def backup_schedule_put(body: AutoBackupScheduleIn, db: Session = Depends(get_db)):
    from honestspend.services.backup import maybe_auto_backup, schedule_status
...
def backup_list():
    from honestspend.services.backup import list_backups
...
def backup_create_encrypted(body: EncryptedBackupIn):
    from honestspend.services.encrypted_backup import create_encrypted_backup
...
def backup_list_encrypted():
    from honestspend.services.encrypted_backup import list_encrypted
...
def backup_decrypt(body: DecryptBackupIn):
    from honestspend.services.backup import backups_dir
    from honestspend.services.encrypted_backup import decrypt_file_to_backup
    from honestspend.services.paths_safe import resolve_under_data_dir, safe_filename
...
def backup_remote_get():
    from honestspend.services.encrypted_backup import get_remote_config
...
def backup_remote_put(body: RemoteBackupConfigIn):
    from honestspend.services.encrypted_backup import set_remote_config
...
def backup_download(name: str):
    from honestspend.services.backup import read_backup_bytes
...
def backup_download_live():
    ...
    from honestspend.services.backup import zip_stream_from_live_db
...
def backup_restore(name: str):
    from honestspend.services.backup import restore_from_backup
...
async def backup_restore_upload(file: UploadFile = File(...)):
    from honestspend.services.backup import restore_from_upload
    from honestspend.services.paths_safe import enforce_upload_size, safe_filename
